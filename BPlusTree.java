...
public class BPlusTree {
	...
	{
		double x = (new Double(degree)+1.0)/2.0;
		D = (int) Math.ceil(x)-1;
		lD = (degree+1)/2;
		n = degree;
	...
	public int find(int key1, int key2) {
		// look for leaf node that should contain key
		LeafNode leaf = findLeafNodeWithKey(root, key1);
		return recursivefind(leaf, key1, key2);
	}
	...
	public Entry<Integer, Node> splitLeafNode(LeafNode leaf) {
		...
		leaf.keys.subList(lD, leaf.keys.size()).clear();
		leaf.values.subList(lD, leaf.values.size()).clear();
	...
	public int handleLeafNodeUnderflow(LeafNode left, LeafNode right,
		...
		// merge
		if (left.keys.size() + right.keys.size() < n){
	...
	public int handleIndexNodeUnderflow(IndexNode leftIndex,
		...
		// Action : merge
		if (leftIndex.keys.size() + rightIndex.keys.size() < n){
